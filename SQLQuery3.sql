...
ORDER BY
	...
	------Having---- its a conditin used to aggrated data.
	--- below is the syntax used to aggregate data
	...
	having (column_two,
	column_three
	column_n) (comparison operator) then condition
	*/

select s.salesorderid,
s.salesorderdetailid,
sum(s.orderqty) total_qty
from [Sales].[SalesOrderDetail] s
group by 
s.salesorderid,
s.salesorderdetailid
having sum(s.orderqty)>2
...
client_Address_name varchar(20),
client_City_name varchar(20),
client_state_name varchar(20)
)

client_first_name,client_last_name,client_Address_name,client_City_name,client_state_name 
...
/* windows function are also know as analystic function.
*/

/*
to understand  the windowsa function one will be needed to go ahead and 
just such a table*/
-------------------------------
use movies
---------------------------------
...
insert into dummy_employees(employee_id,employee_name,dept_name,salary) values
(11,'prika','analyst',4500),
(12,'Gadham','analyst',3500),
(13,'Gadham','analyst',2500)
...
--------------------------------------
lets say we want to find the maximum salary of the employees
--------------------------------------------------

select max(salary) as  maximum_salary, dept_name  from dummy_employees
group by dept_name

-------------------------------------------

select  min (salary) min_salary,dept_name from dummy_employees
group by dept_name
order  by min (salary)  asc

-------------------------------------------
find dept with highest salary
-----------------------------
select e.*, max(salary) over() as  max_salary from dummy_employees e

------------------------------------
/* when the code will be executed, the max() will be used as a windows function and over by will be open a new
window with  max_salary */



/*But if we want to extract max salary from department will have to partiotion the column we need*/

select e.*, max(salary) over(partition by dept_name) as max_salary
from dummy_employees e


------------Other windows function in sql server-----------------------------

/*in sql server we normally have other window function such as row_number,dense rank,rank ,lag and lead*/

let say we want to know how many records does our departments have 

select e.*, row_number() over(partition by dept_name order by dept_name asc) as rn from dummy_employees e



--fetch first 2  employee from each depertment who join the company

select * from(
select e.*, row_number() over (partition by dept_name order by employee_id) as rn 
from dummy_employees e) x
where x.employee_id < 3

------Rank fuction
-------Let say we need top 3 employees from each dept earning max salary
select * from
(
select e.*,rank() over(partition by dept_name order by salary asc) rnk from dummy_employees e
) x where x.rnk < 3



------------------------Dense Rank
select e.*,rank() over(partition by dept_name order by salary asc) rnk,
dense_rank() over(partition by dept_name order by salary asc) dense_rnk,
row_number() over(partition by dept_name order by salary desc) as row_number from dummy_employees e



-------Lag and Led, lets say that you want to get the employees previous salary we can go ahead and use this two.
select e.*,
lag(salary)  over(partition by dept_name order by employee_id) previous_salary
from dummy_employees e


/*on lag and led, one can go ahead  and know the previous by specifying the number*/
select e.*,
lag(salary,2,0) over(partition by dept_name order by employee_id) previus_salary
from dummy_employees e

/* we will be looking at led, led will be looking for next record after the actual one.*/
select e.*,
lag(e.salary) over(partition by dept_name order by employee_id) previous_employees_salary,
case 
when e.salary >lead(e.salary) over(partition by dept_name order by employee_id) then 'Greater salary'
 when e.salary = lead(e.salary) over(partition by dept_name order by employee_id) then 'equal salary'
end salary_range
from dummy_employees e



------------Loading data to server based on the latest date.
use movies
-----------------------------------------------------------
select * from dummy_employees
------------------------------------------------------------
...
insert into dept(dept_id,dept_name,employee_id) values (1,'Admin',1),
(2,'IT',2),
(3,'Hr',3),
(4,'Admin',4),
(5,'Engineer',5),
...
select * from dept d, dummy_employees e
...
truncate table dummy_employees

-------------------------------------------------

select * from [dbo].[data_sample]

insert into fileinfo values('filepath','recordcount','date')
...
CREATE TABLE sales_customers_tgt(
	[customer_id] [int] IDENTITY(1,1) NOT NULL,
	[first_name] [varchar](255) NOT NULL,
	[last_name] [varchar](255) NOT NULL,
	[phone] [varchar](25) NULL,
	...
	alter table sales_customers_tgt
	Add city_shorten varchar(5) 

	select * from sales_customers_tgt
	...
	Alter table sales_customers_tgt
	add  Full_name nvarchar(90)

	--------------
	Alter table sales_customers_tgt
	add  State_name nvarchar(90)
	---------------------
	...
	-------------------------------------------
	CREATE TABLE sales_customers_src(
	[customer_id] [int] IDENTITY(1,1) NOT NULL,
	[first_name] [varchar](255) NOT NULL,
	[last_name] [varchar](255) NOT NULL,
	[phone] [varchar](25) NULL,
...
CREATE TABLE sales_customers_All(
	[customer_id] [int] IDENTITY(1,1) NOT NULL,
	[first_name] [varchar](255) NOT NULL,
	[last_name] [varchar](255) NOT NULL,
	[phone] [varchar](25) NULL,
	...
	use Demossis
	select  distinct * from  sales_customers_All
	where phone is not null

	drop table  students_demo
	create table students_demo
	(
	Student_id int not null,
	students_name varchar(90) null,
	students_gender varchar(90) null
	) 
	
	insert into students_demo(Student_id,students_name,students_gender) values(1,'John','Male'),
	(2,'Rick','Male'),
	(3,'Johann','Female'),
	(4,'Grace','Female')
	...
	) insert into products(product_id,product_name) values(1,'cars'),
	(2,'phone'),
	(3,'charger'),
	(4,'jack')

   --------------------------------
   CREATE PROCEDURE updtproducts    
      AS
        BEGIN
         UPDATE products 
         SET product_name='socket' 
         WHERE product_id=4
        END

		Execute  updtproducts

		drop procedure updtproducts
